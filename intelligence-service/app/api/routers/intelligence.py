...
def trigger_risk_update():
    """Simulate XGBoost risk score change triggering an event via analytics.predictions topic"""
    event = {
    ...
    producer.flush()
    return {"status": "success", "message": "zone:risk:update event pushed to Kafka", "payload": event}

@router.post("/trigger-prediction-new")
def trigger_prediction_new():
    """Simulate ML pipeline finishing evaluation and triggering a new prediction"""
    event = {
    ...
    producer.send("analytics.predictions", event)
    producer.flush()
    return {"status": "success", "message": "prediction:new event pushed to Kafka", "payload": event}

@router.post("/trigger-alert-new")
def trigger_alert_new():
    """Simulate ML pipeline triggering a new alert when prediction crosses severity threshold"""
    event = {
        "event": "alert:new",
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "data": {
    ...
    producer.flush()
    return {"status": "success", "message": "alert:new event pushed to Kafka", "payload": event}

@router.post("/trigger-alert-resolved")
def trigger_alert_resolved():
    """Simulate Admin standing down an emergency alert"""
    event = {
    ...
    producer.send("system.alerts", event)
    producer.flush()
    return {"status": "success", "message": "alert:resolved event pushed to Kafka", "payload": event}
def trigger_prediction_new():
    """Simulate ML pipeline finishing evaluation and triggering a new prediction"""
    event = {
        "event": "prediction:new",
